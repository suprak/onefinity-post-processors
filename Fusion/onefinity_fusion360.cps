...
function onSection() {
    ...
    if (workOffset > 0) {
        if (workOffset > 9) {
            ...
            if (workOffset != currentWorkOffset) {
                ...
                if (workOffset > 6) {
                    writeBlock(gFormat.format(59 + ((workOffset - 6) / 10)), wcsComment); // G59.1->G59.3
                } else {
    ...
    if (insertToolCall || retracted) {
        ...
        // blocks like `G43 Z0.6 H1` probably should not be output, since tool registers are not kept by buildbotics?
